...
if mFea/nSmp > 1.0713  % 如果X的m大
    ...
    if (ReducedDim > 0) && (dimMatrix > MAX_MATRIX_SIZE) && (ReducedDim < dimMatrix*EIGVECTOR_RATIO)
        ...
        U = U(:, index);  % 特征值和向量由大到小进行排序， 即为左奇异向量
    end
    clear ddata;
    ...
    eigvalue(eigIdx) = [];
    U(:,eigIdx) = [];   % 去掉为0的特征值的，及对应的向量
    
    if (ReducedDim > 0) && (ReducedDim < length(eigvalue))
        eigvalue = eigvalue(1:ReducedDim);
        U = U(:,1:ReducedDim);     % 取前 ReducedDim 个最大的特征值及向量
    end
    
    eigvalue_Half = eigvalue.^.5;   % 每个特征值开根号，为奇异值
    S =  spdiags(eigvalue_Half,0,length(eigvalue_Half),length(eigvalue_Half)); 

    if nargout >= 3
        eigvalue_MinusHalf = eigvalue_Half.^-1;  % 奇异值每个元素求逆
        V = X'*(U.*repmat(eigvalue_MinusHalf',size(U,1),1)); % repmat(eigvalue_MinusHalf',size(U,1),1) 每行为奇异值每个元素求逆，共size(U,1)行
    end  % V= X'*(SU')^-1 ,U'U=I,U'=U^-1,                                                    %   U的每一列乘以对应位置的（奇异值）^-1
else    % 如果X的n大
    ddata = X'*X; % m*m
    ddata = max(ddata,ddata');
